...
export class FormUserComponent {
  ...
  store() {
    ...
    this.UserService.store(form).subscribe((rs) => {
      if (rs.isError) {
        this.isSending = false;
        this.AppComponent.alert({ summary: "Operacion fallida", detail: rs.message, severity: 'error' });
      } else {
        this.router.navigate(['/admin/users']);
        this.AppComponent.alert({ summary: "Operacion exitosa", detail: rs.message, severity: 'success' });
      }
    });
